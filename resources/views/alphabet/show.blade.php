<x-layouts.app>
    ...
    <div class="space-y-8 max-w-4xl mx-auto">
        <!-- Breadcrumb & Nav -->
        <div class="flex flex-wrap items-center justify-between gap-2.5 text-xs text-[#687076] dark:text-[#94A3B8]">
            <div class="flex items-center gap-1.5 sm:gap-2">
                <a href="{{ route('alphabet.index') }}" class="hover:text-[#1A1D20] dark:hover:text-white transition-colors">&larr; {{ __('Alphabet') }}</a>
                <span>/</span>
        ...
        <div class="rounded-3xl bg-white dark:bg-[#131926] border border-[#EBE6DE] dark:border-[#212B3E] p-6 sm:p-10 shadow-xs flex flex-col items-center text-center relative overflow-hidden">
            <!-- Order Badge -->
            <span class="sm:absolute sm:top-6 sm:left-6 text-xs font-mono font-medium px-2.5 py-1 rounded-full bg-[#FAF8F5] dark:bg-[#0B0F19] text-[#687076] dark:text-[#94A3B8] border border-[#EBE6DE] dark:border-[#212B3E] mb-2 sm:mb-0">
                Letter {{ $letter->order }} of 28
            </span>

            <!-- Huge Arabic Glyph (Clicking plays audio) -->
            <button @click="playAudio('{{ $letter->audio_url }}', '{{ $letter->name_ar }}')" type="button" class="my-2 sm:my-4 font-arabic text-7xl sm:text-9xl text-[#1A1D20] dark:text-white leading-tight selection:bg-transparent hover:scale-105 active:scale-95 transition-transform cursor-pointer focus:outline-hidden" title="{{ __('Click to listen') }}: {{ $letter->name_ar }}">
                {{ $letter->character }}
            </button>

            <h1 class="text-xl sm:text-2xl font-bold text-[#1A1D20] dark:text-white tracking-tight flex flex-wrap items-center justify-center gap-2 sm:gap-3">
                <span>{{ app()->getLocale() === 'bn' && $letter->name_bn ? $letter->name_bn : $letter->name_latin }}</span>
                <span class="font-arabic text-xl sm:text-2xl text-[#1B4D3E] dark:text-emerald-400 font-normal">({{ $letter->name_ar }})</span>
            </h1>

            <p class="text-xs sm:text-sm font-medium text-[#687076] dark:text-[#94A3B8] mt-1">
            ...
            <!-- Audio Trigger -->
            <button @click="playAudio('{{ $letter->audio_url }}', '{{ $letter->name_ar }}')" type="button" class="mt-4 sm:mt-5 inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-[#1B4D3E] dark:bg-emerald-600 text-white text-xs font-semibold hover:bg-[#153e32] dark:hover:bg-emerald-500 shadow-xs hover:scale-105 transition-all">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            <div class="rounded-2xl bg-white dark:bg-[#131926] border border-[#EBE6DE] dark:border-[#212B3E] p-4 sm:p-6 space-y-2">
