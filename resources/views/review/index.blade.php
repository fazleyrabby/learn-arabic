<x-layouts.app>
    ...
    <div x-data="{
        ...
        <template x-if="!completed && currentCard">
            <div class="space-y-6">
                <!-- Card Body -->
                <div class="min-h-[300px] sm:min-h-[360px] rounded-3xl bg-white dark:bg-[#131926] border border-[#EBE6DE] dark:border-[#212B3E] p-6 sm:p-8 flex flex-col items-center justify-between text-center shadow-xs transition-all">
                    <!-- Card Type Tag -->
                    <span class="text-[11px] font-medium tracking-wider uppercase text-[#687076] dark:text-[#94A3B8]" x-text="currentCard.sub"></span>

                    <!-- Prompt (Front) -->
                    <div class="my-4 sm:my-6">
                        <div class="font-arabic text-6xl sm:text-8xl text-[#1A1D20] dark:text-white leading-relaxed select-none" x-text="currentCard.title"></div>
                    </div>

                    <!-- Answer (Back) -->
                    <div class="w-full">
                        <div x-show="!revealed">
                            <button @click="reveal()" type="button" class="w-full py-3 rounded-2xl bg-[#FAF8F5] dark:bg-[#0B0F19] border border-[#EBE6DE] dark:border-[#212B3E] text-xs font-semibold text-[#1A1D20] dark:text-white hover:border-[#1B4D3E] dark:hover:border-emerald-500 transition-colors cursor-pointer">
                                Show Answer
                            </button>
                        </div>
                ...
                <div x-show="revealed" x-transition class="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-2">
                    <button @click="submitGrade(1)" :disabled="submitting" class="p-3 rounded-xl bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-900/50 hover:bg-red-100 dark:hover:bg-red-900/50 text-center transition-colors cursor-pointer">
                    ...
                    <button @click="submitGrade(2)" :disabled="submitting" class="p-3 rounded-xl bg-orange-50 dark:bg-orange-950/30 border border-orange-200 dark:border-orange-900/50 hover:bg-orange-100 dark:hover:bg-orange-900/50 text-center transition-colors cursor-pointer">
                    ...
                    <button @click="submitGrade(3)" :disabled="submitting" class="p-3 rounded-xl bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-900/50 hover:bg-emerald-100 dark:hover:bg-emerald-900/50 text-center transition-colors cursor-pointer">
                    ...
                    <button @click="submitGrade(4)" :disabled="submitting" class="p-3 rounded-xl bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-900/50 hover:bg-blue-100 dark:hover:bg-blue-900/50 text-center transition-colors cursor-pointer">
        ...
        <template x-if="completed || cards.length === 0">
            <div class="rounded-3xl bg-white dark:bg-[#131926] border border-[#EBE6DE] dark:border-[#212B3E] p-6 sm:p-10 text-center space-y-4 shadow-xs">
                ...
                <div class="pt-4 flex flex-col sm:flex-row items-center justify-center gap-3">
                    <a href="{{ route('dashboard') }}" class="w-full sm:w-auto px-5 py-2.5 rounded-xl bg-[#1B4D3E] dark:bg-emerald-600 text-white text-xs font-semibold hover:opacity-90 transition-opacity text-center">
                        Back to Dashboard
                    </a>
                    <a href="{{ route('quran.fatihah') }}" class="w-full sm:w-auto px-5 py-2.5 rounded-xl bg-[#FAF8F5] dark:bg-[#0B0F19] border border-[#EBE6DE] dark:border-[#212B3E] text-xs font-semibold text-[#1A1D20] dark:text-white hover:bg-[#EAE5DB]/50 transition-colors text-center">
                        Study Al-Fatihah
                    </a>
                </div>
            </div>
